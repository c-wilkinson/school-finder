...
def _lookup_result(*, current=True, termination=None):
    request = SchoolSearchRequest("RG22 6SX")
    postcode = PostcodeLocation("RG22 6SX", 1, 2, current, termination)
    school = SchoolResult(SchoolIdentity("100001", "Example"))
    flat = ({
        "school_name":"Example", "distance_miles":1.2, "sector":"State-funded",
        "establishment_type":"Academy", "age_range":"11–16", "ofsted_rating":"Good",
        "ofsted_inspection_date":"2025-01-01", "attainment8":50.0, "progress8":0.1,
        "progress8_year":"202324", "town":"Town", "postcode":"RG1 1AA", "urn":"100001",
    },)
    return SchoolSearchResult(request, postcode, (school,), flat)


def _lookup_args(**overrides):
    values = dict(data_dir=Path("data"), postcode="RG22 6SX", limit=20, entry_age=11, minimum_exit_age=16, include_special=False, json=False, structured_json=False)
...
def test_create_parser_build_and_lookup_defaults():
    ...
    lookup = parser.parse_args(["lookup", "RG22 6SX"])
...
def test_parser_rejects_mutually_exclusive_json_flags():
    ...
    with pytest.raises(SystemExit):
        parser.parse_args(["lookup", "RG22 6SX", "--json", "--structured-json"])
...
def test_run_lookup_table_output(monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Nearest 1 schools" in out
...
def test_run_lookup_passes_all_search_request_fields(monkeypatch):
    ...
    cli._run_lookup(_lookup_args(limit=5, entry_age=10, minimum_exit_age=18, include_special=True, json=True))
    assert seen["request"] == SchoolSearchRequest("RG22 6SX", 5, 10, 18, True)
...
def test_main_dispatches_lookup(monkeypatch):
    monkeypatch.setattr(sys, "argv", ["school-finder", "lookup", "RG22 6SX"])
    monkeypatch.setattr(cli, "_run_lookup", lambda args: 8)
    assert cli.main() == 8


@pytest.mark.parametrize("exc", [SchoolFinderError("bad"), OSError("bad"), ImportError("bad")])
